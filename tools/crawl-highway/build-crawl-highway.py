#!/usr/bin/env python3
"""
SideGuy Crawl Highway Builder
Generates premium hub pages from mega-hubs.txt + cluster-map.csv
Run: python3 tools/crawl-highway/build-crawl-highway.py
"""
import os
import csv
from datetime import date

HUBS_FILE  = "data/crawl-highway/mega-hubs.txt"
MAP_FILE   = "data/crawl-highway/cluster-map.csv"
REPORT     = "docs/crawl-highway/crawl-highway-report.md"
LOG        = "logs/crawl-highway/build.log"
OUTDIR     = "pages/hubs"
SITEMAP    = "sitemap.xml"
MONTH      = "March 2026"
...
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px;margin:1rem 0;}
.grid a{display:block;padding:12px 16px;border-radius:14px;background:rgba(255,255,255,.75);border:1px solid rgba(7,48,68,.09);color:var(--accent);text-decoration:none;font-weight:600;font-size:.93rem;box-shadow:0 2px 8px rgba(7,48,68,.06);}
.grid a:hover{background:#fff;box-shadow:0 4px 16px rgba(7,48,68,.1);}
...
.hubs-nav{display:flex;flex-wrap:wrap;gap:8px;margin:1.2rem 0;}
.hubs-nav a{display:inline-block;padding:8px 14px;border-radius:var(--pill);border:1px solid rgba(7,48,68,.12);background:rgba(255,255,255,.65);font-size:.82rem;color:var(--ink);text-decoration:none;}
...
def build_page(hub, children, all_hubs):
    title = tc(hub)
    child_links = "\n".join(
        f'      <a href="/{c}.html">{tc(c)}</a>' for c in children
    )
    other_hubs = "\n".join(
        f'      <a href="/hubs/{h}.html">{tc(h)}</a>'
        for h in all_hubs if h != hub
    )
    faq_entries = ""
    if children:
        faq_entries = f"""    {{
      "@type": "Question",
      "name": "What topics does the {title} hub cover?",
      "acceptedAnswer": {{
        "@type": "Answer",
        "text": "The {title} hub covers {len(children)} related topics including {', '.join(tc(c) for c in children[:3])}{',' and ' and more' if len(children) > 3 else ''}. Each linked page provides plain-language guidance on that specific problem or service area."
      }}
    }},
    {{
      "@type": "Question",
      "name": "How does SideGuy help with {title.lower()} questions?",
      "acceptedAnswer": {{
        "@type": "Answer",
        "text": "SideGuy provides human-first guidance on {title.lower()} — no sales pitch, no vendor kickbacks. Read the guides to understand your options, then text PJ at 773-544-1231 if you want a real second opinion on your specific situation."
      }}
    }}"""
...
<head>
  ...
  <title>{title} Hub · SideGuy Solutions</title>
  <link rel="canonical" href="https://sideguysolutions.com/hubs/{hub}.html"/>
  <meta name="description" content="SideGuy {title} hub — plain-language guides, problem pages, and clarity-first guidance for operators and homeowners."/>
  <meta property="og:title" content="{title} Hub · SideGuy"/>
  <meta property="og:description" content="SideGuy {title} hub — real guidance on {title.lower()}, no sales pitch."/>
  <meta property="og:url" content="https://sideguysolutions.com/hubs/{hub}.html"/>
...
{{
  ...
  "itemListElement": [
    ...
    {{"@type": "ListItem", "position": 2, "name": "{title} Hub", "item": "https://sideguysolutions.com/hubs/{hub}.html"}}
  ]
}}
</script>
<style>
...
<p style="font-size:.8rem;opacity:.6;margin-bottom:4px;">SideGuy Solutions · Updated {MONTH}</p>
<h1>{title}</h1>
<p class="lede">SideGuy organizes everything we know about {title.lower()} in one place — so you can find the right guide, understand your options, and avoid expensive mistakes.</p>

<p>This hub exists for two audiences: people trying to understand a problem, and search engines trying to understand how our content is connected. Every linked page provides direct, practical guidance with no sales pitch attached.</p>
...
<div class="cta-block">
  <div>
    <p class="t">Stuck on a specific situation?</p>
    <p class="s">Text PJ — real answer, no pitch. 773-544-1231</p>
  </div>
  <a class="btn" href="sms:+17735441231">📱 Text PJ</a>
</div>

<h2>About This Hub</h2>
<p>SideGuy is a human-first guidance platform based in San Diego. We build clear, accurate reference pages for real problems — not keyword-stuffed filler. The {title} hub is part of SideGuy's effort to map every significant problem and service area so operators and homeowners can find plain-language answers before spending money.</p>
<p>Every page in this hub is written to answer a specific question: what does this cost, what are the warning signs, who do I call, and how do I avoid being overcharged. If you have a question not covered here, text PJ at 773-544-1231.</p>

<h2>Other SideGuy Hubs</h2>
<div class="hubs-nav">
{other_hubs}
...
def main():
    ...
    # Load hubs
    all_hubs = [l.strip() for l in open(HUBS_FILE) if l.strip()]
    ...
    with open(MAP_FILE) as f:
        ...
        for row in reader:
            h = row["hub"].strip()
            c = row["child"].strip()
            cluster.setdefault(h, []).append(c)

    report_lines = ["# SideGuy Crawl Highway Report", "", f"Build: {date.today()}", ""]
    new_urls = []
    count = 0

    for hub in all_hubs:
        children = cluster.get(hub, [])
        html = build_page(hub, children, all_hubs)
        filepath = os.path.join(OUTDIR, f"{hub}.html")
        open(filepath, "w").write(html)

        url = f"https://sideguysolutions.com/hubs/{hub}.html"
        new_urls.append(url)
        count += 1

        report_lines += [f"## {hub}", "", f"- Hub page: `/hubs/{hub}.html`", "- Children:"]
        for c in children:
            report_lines.append(f"  - `/{c}.html`")
        report_lines.append("")

    report_lines += ["Build complete."]
    open(REPORT, "w").write("\n".join(report_lines) + "\n")
    ...
    with open(LOG, "a") as f:
        f.write(f"Build finished {date.today()}: {count} hubs\n")

    print(f"\nHub pages generated: {count}")
